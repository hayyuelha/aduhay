		...
		if ($taman){
			foreach($taman as $tmn){
				...
				foreach ($kategori as $kat) {
					...
					$row['menunggu'] = $this->db->query("SELECT COUNT(DISTINCT id) AS num FROM aduan WHERE id_status = 1 AND YEAR(waktu) = ".$this->tahun." AND MONTH(waktu) = ".$this->bulan." AND id_taman = ".$tmn['id']." AND id_kategori = ".$kat['id'])[0]['num'];
	...
	public function getLaporanByStatus(){
		...
		if ($taman){
			foreach ($taman as $row) {
				$temp = $row;
				$temp['menunggu'] = $this->db->query("SELECT COUNT(DISTINCT id) AS num FROM aduan WHERE id_status = 1 AND YEAR(waktu) = ".$this->tahun." AND MONTH(waktu) = ".$this->bulan." AND id_taman = ".$row['id'])[0]['num'];
